...
var parseNbResult = function ($) {
    return parseInt($('nav > .navlist.type > .selected > .value > b').text().replace(/ /g, ''));
};

var convertStringDateToDate = function (dates) {
    const date = new Date();
...
var parseDate = function ($) {
    const dates = [];
    var elems = $.find('.date div');
    elems.each(function (index) {
        dates.push(cleanString(elems.eq(index).text()));
    });
    return convertStringDateToDate(dates)
};

var parseImages = function ($) {
    const images = []
    var elems = $.find('.image img');
    elems.each(function (index) {
...
var parseCategory = function ($) {
    return cleanString($.find('.category').text())
};
...
var parseLocation = function ($) {
    return cleanString($.find('.placement').text())
};

var parseIsUrgent = function ($) {
    return $.find('.urgent').length > 0
};

var parsePrice = function ($) {
    return parseInt(cleanString($.find('.price').text().replace(/ /g, '')))
};
